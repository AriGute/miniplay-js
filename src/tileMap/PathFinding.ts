...
export class PathFinding {
	private static tileMap: TileBaseType[][] = null;
	private static start: LeanPoint = null;
	private static target: LeanPoint = null;
	private static MAX_SIZE_X: number = 0;
	private static MAX_SIZE_Y: number = 0;
	private static walkableTiles: TileBaseType[] = [TileBaseType.ground];

	public static a_star(
		tileMap: TileBaseType[][],
		walkableTiles: TileBaseType[],
		normalizedStartPosition: LeanPoint,
		normalizedTargetPosition: LeanPoint,
		...
		const scoreMap: MinTileHeap = new MinTileHeap();
		...
		let relativeTile: TileScore = {
			x: normalizedStartPosition.x,
			y: normalizedStartPosition.y,
			score: 0,
			prevTile: null,
		};
		let currentTile: TileScore = null;
				...
				for (let x = -1; x <= 1; x++) {
					...
					if (currentTile.x === this.target.x && currentTile.y === this.target.y) {
						...
						while (currentTile) {
							...
							if (config.debugMode.pathFinding) debugPath.push({ x: currentTile.x, y: currentTile.y });
						...
						if (config.debugMode.pathFinding) {
							...
							debugPath.forEach((t) => {
								...
								PathFinding.tileMap[t.y][t.x] = TileBaseType.default;
							});
							setTimeout(() => {
	...
	/**
	 * straight line with no obstacles in the way (no tile walls, only walkable)
	 * @param p1 point in space (not tile point)
	 * @param p2 point in space (not tile point)
	 * @param scene current played scene (for tileMap internals methods reference)
	 * @param walkableTiles define what is "clear", if  one of the tiles between p1 and p2 are not from this set then return false.
	 */
	public static clearLine(scene: Scene, p1: LeanPoint, p2: LeanPoint, walkableTiles?: TileBaseType[]) {
		...
		if (dist.x < dist.y) {
			...
			while (currentPos.x < targetPos.x) {
				...
				if (!(walkableTiles || this.walkableTiles).includes(map.tileMap[currentPos.y][currentPos.x])) return false;
			}
		} else {
			let ratioCycle = 0;
			...
			if (scene.tileMap.isTileOnMap(currentPos) && !(walkableTiles || this.walkableTiles).includes(map.tileMap[currentPos.y][currentPos.x])) return false;
		}
		return true;
	}

	private static unNormalizePosition(position: LeanPoint): LeanPoint {
		return {
	...
	private static isTileOnMap(tile: TileScore) {
		if (tile.x < 0) return false;
		if (tile.y < 0) return false;
		if (tile.x > PathFinding.MAX_SIZE_X - 1) return false;
		if (tile.y > PathFinding.MAX_SIZE_Y - 1) return false;
		return true;
	}

	private static isTileWalkable(tile: TileScore): boolean {
		return PathFinding.walkableTiles.includes(PathFinding.tileMap[tile.y][tile.x]);
	}

	private static calcTileValue(toCalc: LeanPoint) {
	...
	public static clear() {
		...
		PathFinding.walkableTiles = [TileBaseType.ground];
	}
}
