...
export class UI {
    ...
    static _font = config.graphics.textStyle.default_font;
    static get font() {
        return UI._font;
    }
    static update() {
    ...
    static drawUIComponents() {
        ...
        for (let i = 0; i <= UI.maxIndex; i++) {
            ...
            if (drawOrderIterator) {
                ...
                for (let i = 0; i < drawOrderIterator.size; i++) {
                    ...
                    if (imgToDraw)
                        UI.context2d.drawImage(imgToDraw.img, 0, 0, imgToDraw.img.width, imgToDraw.img.height, (imgToDraw.position.x * config.graphics.scaledResolution.width) / config.graphics.targetResolution.width, (imgToDraw.position.y * config.graphics.scaledResolution.height) / config.graphics.targetResolution.height, (imgToDraw.img.width * config.graphics.scaledResolution.width) / config.graphics.targetResolution.width, (imgToDraw.img.height * config.graphics.scaledResolution.height) / config.graphics.targetResolution.height);
                }
            }
        }
        ...
        for (let i = 0; i < UI.uiTextsComponents.size; i++) {
            ...
            if (textToDraw) {
                ...
                UI.context2d.fillText(textToDraw.text, (textToDraw.position.x * config.graphics.scaledResolution.width) / config.graphics.targetResolution.width, (textToDraw.position.y * config.graphics.scaledResolution.height) / config.graphics.targetResolution.height, config.graphics.textStyle.default_max_length);
            }
        }
        ...
        for (let i = 0; i < UI.uiRectComponents.size; i++) {
            ...
            if (rectToDraw) {
                ...
                UI.context2d.rect((rectToDraw.position.x * config.graphics.scaledResolution.width) / config.graphics.targetResolution.width, (rectToDraw.position.y * config.graphics.scaledResolution.height) / config.graphics.targetResolution.height, (rectToDraw.width * config.graphics.scaledResolution.width) / config.graphics.targetResolution.width, (rectToDraw.height * config.graphics.scaledResolution.height) / config.graphics.targetResolution.height);
            }
        }
        UI.context2d.stroke();
        UI.context2d.strokeStyle = 'black';
