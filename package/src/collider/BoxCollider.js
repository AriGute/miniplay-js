...
export class BoxCollider {
    ...
    static checkGlobalCircleCollision(point, radius, targetHitBox = false) {
        ...
        for (const otherGameObjectCollider of !targetHitBox ? BoxCollider.colliders : BoxCollider.hitBoxColliders) {
            if (otherGameObjectCollider.owner.hasTag('ignoreCollider'))
                continue;
    ...
    static checkGlobalPointCollision(pointToTest, targetHitBoxColliders = true, targetActiveBoxCollider = false, tileMapHitTest = false, tileMapFilter) {
        ...
        if (tileMapHitTest) {
            ...
            }, { width: config.graphics.tileMap.tileSize, height: config.graphics.tileMap.tileSize }, tileMapFilter);
        }
        return hit;
    }
    static checkTileMapCollision(pointToTest, tileMapFilter) {
        const normalizedPoint = {
            x: pointToTest.x + config.graphics.tileMap.tileSize,
            y: pointToTest.y - config.graphics.tileMap.tileSize,
        };
        let hit = BoxCollider.tileMapCollideFunction(normalizedPoint, { width: config.graphics.tileMap.tileSize, height: config.graphics.tileMap.tileSize }, tileMapFilter);
        return hit;
    }
