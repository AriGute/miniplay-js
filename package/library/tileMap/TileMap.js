...
class TileMap {
    ...
    _startPoint = null;
    get startPoint() {
        return new Point_1.Point(this._startPoint.x * config_1.config.graphics.tileMap.tileSize, this._startPoint.y * config_1.config.graphics.tileMap.tileSize, true);
    }
    _spawnPoint = null;
    get spawnPoint() {
    ...
    _endPoint = null;
    get endPoint() {
        return new Point_1.Point(this._endPoint.x * config_1.config.graphics.tileMap.tileSize, this._endPoint.y * config_1.config.graphics.tileMap.tileSize);
    }
    rooms = [];
    ...
    TILE_SIZE = config_1.config.graphics.tileMap.tileSize;
    scene;
    SCALED_TILE_SIZE_X = Math.round(config_1.config.graphics.tileMap.tileSize);
    SCALED_TILE_SIZE_Y = Math.round(config_1.config.graphics.tileMap.tileSize);
    constructor(scene, maxX, maxY, objectId = (0, utils_1.generateId)(), initiateData) {
        this.scene = scene;
        ...
        if (config_1.config.debugMode.tileMap) {
    ...
    prepareNextDraw(context2d) {
        const minX = Math.floor(Camera_1.Camera.position.x / this.SCALED_TILE_SIZE_X);
        const minY = Math.floor(Camera_1.Camera.position.y / this.SCALED_TILE_SIZE_Y);
        const maxX = Math.ceil((Camera_1.Camera.position.x + config_1.config.graphics.targetResolution.width) / this.SCALED_TILE_SIZE_X);
        const maxY = Math.ceil((Camera_1.Camera.position.y + config_1.config.graphics.targetResolution.height) / this.SCALED_TILE_SIZE_Y);
        ...
        this.backGroundContext2d.fillStyle = config_1.config.graphics.backGroundColor;
        this.backGroundContext2d.fillRect(0, 0, config_1.config.graphics.targetResolution.width, config_1.config.graphics.targetResolution.height);
        for (let y = relativeMinY; y < relativeMaxY; y++) {
            for (let x = relativeMinX; x < relativeMaxX; x++) {
                ...
                if (img) {
                    const position = new Point_1.Point(x * frameCut.width, y * frameCut.height, true);
                    context2d.drawImage(img, frameCut.sourceX, frameCut.sourceY, frameCut.width, frameCut.height, position.x, position.y, this.SCALED_TILE_SIZE_X, this.SCALED_TILE_SIZE_Y);
                }
                else {
                    this.getTile(x, y);
                    return;
    ...
    generateRandomMap(mapConfig) {
        ...
        for (let i = 0; i < mapConfig.numOfRooms; i++) {
            for (let j = 0; j < 5; j++) {
                ...
                if (!this.rooms.some((r) => (0, utils_1.manhattanDistance)(r.pos, { x: randX, y: randY }) < (mapConfig.maxRoomsSize - j) * (mapConfig.maxRoomsSize - j) + 1)) {
                    this.rooms.push({
                        paths: [],
                        pos: {
                            x: randX,
        ...
        for (let i = 0; i < this.rooms.length; i++) {
            ...
            for (let j = 0; j < numOfPaths; j++) {
                ...
                if (room === pickRandomRoom)
                    continue;
                const path = PathFinding_1.PathFinding.a_star(this._tileMap, [-1], room.pos, pickRandomRoom.pos).map((step) => this.normalizePosition(step));
        ...
        if (mapConfig.river) {
            ...
            const riverPath = PathFinding_1.PathFinding.a_star(this._tileMap, [-2, -3, -1], distList[0].pos, distList.at(-1).pos).map((step) => this.normalizePosition(step));
    ...
    *spawnPointGenerator() {
        const spawnRadius = config_1.config.graphics.tileMap.tileSize / 2;
        while (true) {
            yield new Point_1.Point(this.startPoint.x - spawnRadius, this.startPoint.y + spawnRadius);
            yield new Point_1.Point(this.startPoint.x + spawnRadius, this.startPoint.y + spawnRadius);
            yield new Point_1.Point(this.startPoint.x + spawnRadius, this.startPoint.y - spawnRadius);
            yield new Point_1.Point(this.startPoint.x - spawnRadius, this.startPoint.y - spawnRadius);
        }
    ...
    findPath(startPosition, targetPosition) {
        let normalizedCurrentPosition = startPosition instanceof Point_1.Point
        ...
        if (config_1.config.debugMode.logs.tileMap) {
            console.clear();
            console.group();
        ...
        return PathFinding_1.PathFinding.a_star(this._tileMap, this.walkableTiles, normalizedCurrentPosition, normalizedTargetPosition);
    }
    ...
    createTileMapCanvas() {
        this.backGroundCanvas = new OffscreenCanvas(config_1.config.graphics.targetResolution.width, config_1.config.graphics.targetResolution.height);
        this.backGroundContext2d = this.backGroundCanvas.getContext('2d', {
            alpha: false,
            desynchronized: true,
        });
        this.backGroundContext2d.imageSmoothingEnabled = config_1.config.graphics.imageSmoothing;
    }
    getDataToSync() {
        const tileMapData = {
