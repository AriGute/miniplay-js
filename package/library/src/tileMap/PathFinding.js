...
const MinTileHeap_1 = require("./MinTileHeap");
...
class PathFinding {
    ...
    static a_star(tileMap, walkableTiles, normalizedStartPosition, normalizedTargetPosition, diagonalPathFinding) {
        ...
        const scoreMap = new MinTileHeap_1.MinTileHeap();
        ...
        for (let counter = 0; counter < 150; counter++) {
            ...
            for (let y = -1; y <= 1; y++) {
                for (let x = -1; x <= 1; x++) {
                    if (x === 0 && y === 0)
                        continue;
                    if ((diagonalPathFinding ?? config_1.config.graphics.tileMap.diagonalPathFinding) === false) {
                        if (x === -1 && y === -1)
                            continue;
                        if (x === -1 && y === 1)
                            continue;
                    ...
                    if (currentTile.x === this.target.x && currentTile.y === this.target.y) {
                        ...
                        while (currentTile) {
                            ...
                            if (config_1.config.debugMode.pathFinding)
    ...
    static unNormalizePosition(position) {
        return {
            x: Math.round(position.x * config_1.config.graphics.tileMap.tileSize + config_1.config.graphics.tileMap.tileSize / 3),
            y: Math.round(position.y * config_1.config.graphics.tileMap.tileSize + config_1.config.graphics.tileMap.tileSize / 3),
        };
    }
    ...
    static calcTileValue(toCalc) {
        return (0, utils_1.manhattanDistance)(toCalc, this.start) + (0, utils_1.manhattanDistance)(toCalc, this.target) * 1.5 + (0, utils_1.manhattanDistance)(toCalc, toCalc);
    }
    static clear() {
