...
const Scene_1 = require("../abstract/Scene");
...
class Particles {
    ...
    timer = 0;
    delay = 1;
    position = new Point_1.Point(0, 0);
    maxCount = 1;
    randomRangeX = 0;
    randomRangeY = 0;
    randomRangeScale = 0;
    ...
    constructor(img, width, height, options) {
        this.particle = new AnimationFrames_1.AnimationFrames('particle', img, width, height, { loop: true });
        this.id = (0, utils_1.generateId)();
        options &&
            Object.keys(options).forEach((key) => {
                if (key === 'position') {
                    this[key] = new Point_1.Point(options[key].x, options[key].y);
                }
                else if (key === 'forceX') {
    ...
    invokePreCycle() {
        for (let i = 0; i < this.maxCount / 3; i++) {
            ...
            piece.position = new Point_1.Point(x, y, true);
    ...
    beforeDrawUpdate() {
        this.frameDeltaTime = Scene_1.Scene.deltaTime / config_1.config.graphics.targetScreenRefreshRate;
    ...
    drawPieces(context2d) {
        ...
        for (let i = 0; i < this.pieces.length; i++) {
            ...
            if (piece.timeToLive > 0 && frame) {
                ...
                piece.position = new Point_1.Point(piece.position.x + this.forceX * this.frameDeltaTime, piece.position.y + this.forceY * this.frameDeltaTime, false, false);
    ...
    createPiece() {
        ...
        return {
            position: new Point_1.Point(this.position.x + Math.random() * (this.randomRangeX * 2) - this.randomRangeX, this.position.y + Math.random() * (this.randomRangeY * 2) - this.randomRangeY),
